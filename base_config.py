...
class NACEConfig(BaseModel):
    ...
    add_noise: Callable[[NACE, jax.Array, jax.Array, Any], jax.Array] | None = None

    hyperparam_sweep: Callable[[jax.Array, optuna.Trial], tuple[NACE, optax.GradientTransformation]] | None = None

    state_convert: Callable[[NACE, jax.Array, jax.Array | None], MatLike] | None = None # optional function to convert model prediction to cv2 render format with a custom logic

    model_config = {"arbitrary_types_allowed": True}

# called from scripts that require a configuration file
def load_configuration() -> NACEConfig:
    ...
    try:
        ...
        valid_config = NACEConfig(**config_vars)
    ...
    # it is treated as if its type is NACEConfig
    return cfg_module # type: ignore

# print info about inputs, parameters and such
def print_model_info(model: NACE, details: bool = False):
    inputs_details = ""
    if details:
