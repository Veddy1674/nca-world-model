...
class GoombaEnv():
    ...
    def reset(self):
        ...
        self.state[self.goombaPos] = 0  # goomba
        
        return self.state, self.goombaOffsetX
    
    def set_goomba_pos(self, pos):
        self.state[self.goombaPos] = 1 # background
        self.goombaPos = pos
        self.state[self.goombaPos] = 0 # goomba
    ...
    def get_circular_offset(goombaOffsetX):
        ...
        return np.array([goombaOffsetX / 2], dtype=np.float32)
...
def make_dataset(env: GoombaEnv, STEPS):
    ...
    infos = np.empty((STEPS, 1), dtype=np.float32)
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(1), desc="Making datasets"):
        ...
        np.savez_compressed(f"mario2/data/goombas_{i}.npz", states=states, actions=actions, infos=infos)